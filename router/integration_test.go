...
func TestJobsDB(t *testing.T) {
	fmt.Println("router test1")
	mockBackendConfig := mocksBackendConfig.NewMockBackendConfig(gomock.NewController(GinkgoT()))
	...
	errorDB.Setup(jobsdb.ReadWrite, false, "proc_error", dbRetention, migrationMode, true, jobsdb.QueryFiltersT{})
	defer errorDB.TearDown()

	router := &HandleT{}

	router.Setup(mockBackendConfig, &jobDB, &errorDB, gaDestinationDefinition, nil)

	time.Sleep(time.Minute)

	t.Run("t", func(t *testing.T) {
		fmt.Println("running router benchmark test")
		var err error
